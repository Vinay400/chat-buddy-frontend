...
function initChat(username) {
    ...
    console.log('Attempting to connect socket with token:', socket.auth.token ? 'present' : 'missing');
...
function addMessageToUI(isOwnMessage, data) {
    ...
    formattedMessage = formattedMessage.replace(/&lt;a href="(.*?)"&gt;(.*?)&lt;\/a&gt;/g, '<a href="$1" target="_blank">$2</a>'); // Links (if sent as HTML-safe input)
...
messageInput.addEventListener('focus', (e) => {
    socket.emit('feedback', { feedback: `${nameInput.value} is typing....`, id: socket.id });
});
